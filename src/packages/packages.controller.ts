...
export class PackagesController {
  ...
  public async updatePackage(
    ...
    const updateResponse = await this.packagesService.updatePackageByID(
      params.id,
      dto,
    );
  ...
  public async deletePackage(
    @Param() params: GetPackageParams,
  ): Promise<ResponsePackagesMapper> {
    const packageInfo = await this.packagesService.findPackageByIdAndDelete(
      params.id,
    );
    return ResponsePackagesMapper.map(packageInfo);
  }
