...
export class CardService {
    ...
    public async getCards(
        ...
        if (pageOptionsDto.search !== '') {
            queryBuilder.andWhere("(card.status like :search OR" +
        ...
        if (pageOptionsDto.status !== '') {
            queryBuilder.andWhere("card.status like :status", { status: `%${pageOptionsDto.status}%` });
        }

        if (pageOptionsDto.createdAt !== '') {
            const createdAtDate = this.commonService.getDateInMySQLFormat(pageOptionsDto.createdAt)
            queryBuilder.andWhere("card.createdAt like :createdAt", { createdAt: `%${createdAtDate}%` });
        }
