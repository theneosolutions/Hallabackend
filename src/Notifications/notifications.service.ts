...
export class NotificationsService {
    ...
    public async create(origin: string | undefined, dto: NotificationDto): Promise<Notifications> {
        const { user: userId, content, resourceId, resourceType, parent, parentType, sendNotificationTo } = dto;
        console.log("🚀 ~ file: notifications.service.ts:48 ~ NotificationsService ~ create ~ dto:", dto)

        if (isNaN(sendNotificationTo) || isNull(sendNotificationTo) || isUndefined(sendNotificationTo)) {
        ...
        console.log("🚀 ~ NotificationsService ~ create ~ notificationItem:", notificationItem)
        ...
        console.log("🚀 ~ file: notifications.service.ts:67 ~ NotificationsService ~ create ~ liveStreamAlertMobile:", JSON.stringify({
        ...
        request({
            ...
            if (error) { console.error(error); }
                ...
                console.error("HTTP Error: " + response.statusCode + " - " + JSON.stringify(response));
            }
            else {
                console.log(body);
            }
    ...
    public async getNotifications(
        ...
        const queryBuilder = this.notificationsRepository.createQueryBuilder("notifications");

        queryBuilder.leftJoinAndSelect('notifications.user', 'user')
    ...
    public async getNotificationsByUserId(
        ...
        queryBuilder.where("notifications.userId = :id", { id: id })
    ...
    public async updateStatus(
        ...
        const notificationInfo = await this.notificationsRepository.findOneBy({ id: id });
        if (!notificationInfo) {
            throw new BadRequestException('notification with id ' + notificationInfo + ' does not exist');
        }
    ...
    public async readAllNotificationByUserId(
        id: number,
    ): Promise<IMessage> {

        await this.notificationsRepository.update({ user: id }, { status: true });
        return this.commonService.generateMessage('All notifications are marked as read!');;
    }
    ...
    public async getNotificationsByResourceId(
        ...
        queryBuilder.where("notifications.resourceId = :id", { id: id })
            .leftJoinAndSelect('notifications.user', 'user')
            .orderBy("notifications.createdAt", pageOptionsDto.order)
            .skip(pageOptionsDto.skip)
            .take(pageOptionsDto.take);
    ...
    public async getNotificationsByResourceType(
        ...
        queryBuilder.where("notifications.resourceType = :type", { type: type })
            .leftJoinAndSelect('notifications.user', 'user')
            .orderBy("notifications.createdAt", pageOptionsDto.order)
            .skip(pageOptionsDto.skip)
            .take(pageOptionsDto.take);
    ...
    public async deleteByNotificationId(id: number): Promise<IResponseNotifications> {
        ...
        const deleteResponse = await this.notificationsRepository.delete(notificationInfo.id);
