...
export class EventsService {
  ...
  public async sendEventInvites(userId: number, id: string): Promise<Events> {
    ...
    if (invitesList.length === 0) {
      throw new BadRequestException(`No contacts found against event`);
    }
  ...
  public async update(
    ...
    try {
      ...
      const currentDate = new Date();
      const eventItemDate = new Date(eventItem.eventDate);
      eventItem.updatedAt = currentDate;
      eventItem.status =
        eventItemDate.getTime() > currentDate.getTime()
          ? 'active'
          : eventItem.status;
