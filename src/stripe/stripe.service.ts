...
export class StripeService {
  ...
  public async refund(payment_intent): Promise<any> {
    return await this.stripe.refunds.create({
      payment_intent: String(payment_intent).split('_secret_')[0],
    });
  }
  ...
  public async createCustomer(name: string, email: string) {
    ...
    if (!stripe_customer) {
      throw new BadRequestException(
        'Error creating new customer on Stripe. Please try again',
      );
    }
  ...
  public async charge(amount: number, email: string, name: string) {
    console.log(
      '🚀 ~ file: stripe.service.ts:54 ~ StripeService ~ charge ~ +(amount * 100).toFixed(2)',
